...
public class Main {
    ...
    public static void testBoth() {
        try {
            ...
            final UserAttributes[] userAttributes = new UserAttributes[1];
            ...
            Thread casThread = new Thread(() -> {
                try {
                    ...
                    // Valider le ticket de service (ST) et extraire les attributs utilisateur
                    userAttributes[0] = authService.getUserAttributes(st, serviceUrl).get();

                    // Indiquer que l'authentification est réussie
                    System.out.println("\n" + ANSI_GREEN + "Authentication CAS successful for user: " + ANSI_RESET + userAttributes[0].getMail());
            ...
            Thread chatGPTThread = new Thread(() -> {
                try {
                    // Attendre que l'authentification CAS soit terminée
                    latch.await();

                    if (!isAuthenticated[0]) {
                        return; // Ne pas exécuter si l'authentification CAS a échoué
                    }
                    ...
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/java/connection_history.txt", true))) {
                        writer.write("User: " + userAttributes[0].getMail() + "\n");
                        writer.write("Client IP Address: " + userAttributes[0].getClientIpAddress() + "\n");
                        writer.write("Authentication Date: " + userAttributes[0].getAuthenticationDate() + "\n");
            ...
            System.out.println("\r" + ANSI_CLEAR_LINE + ANSI_RED + "Error : Authentication failed" + ANSI_RESET);
        }
    }
}
